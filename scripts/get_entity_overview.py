...
def get_player_mappings(entities_dir: Path) -> Dict[int, Dict[str, str]]:
    """Extract player to character mappings from players.md file."""
    ...
    if players_file.exists():
        ...
        for player_name, entity_id in matches:
            mappings[int(entity_id)] = {'player': player_name}
...
def format_overview(overview: Dict[str, List[Dict]], player_mappings: Dict[int, Dict[str, str]]) -> str:
    """Format the overview into a readable summary."""
    output = ["# Entity Overview\n"]
    
    # First show player character mappings
    if player_mappings:
        output.append("## Player Characters")
        for entity_id, info in player_mappings.items():
            # Find the character in our overview
            for entity in overview.get('characters', []):
                if entity.get('entity_id') == entity_id:
                    output.append(f"{info['player']} plays {entity['name']} - entities/{entity['file']}")
                    break
        output.append("")
...
def main():
    ...
    entity_types = ['notes', 'characters', 'locations', 'organizations', 'races']
    
    # Get player mappings first
    player_mappings = get_player_mappings(entities_dir)
    ...
    # Format and output
    formatted = format_overview(overview, player_mappings)
    print(formatted)
